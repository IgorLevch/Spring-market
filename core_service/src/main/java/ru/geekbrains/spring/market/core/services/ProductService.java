...
public class ProductService {
    ...
    public Page<Product> findAll(Specification<Product> spec, int page){
        ...
        return productRepository.findAll(spec, PageRequest.of(page,5)); // по 5 продуктов на странице
    }
    ...
    public Specification<Product>  createSpecByFilters(Integer minPrice, Integer maxPrice, String title ){
        ...
        Specification<Product> spec = Specification.where(null); // данная специфиация не накладывает ограничений в СКьюЛьном запросе
            if (minPrice != null){   // spec - иммутабельный

                spec = spec.and(ProductsSpecifications.priceGreaterOrEqualThan(minPrice));
